...
#   bash scripts/sweep_step3.sh
#
# Estimated time: ~30s/run on GPU, ~3–5 min/run on CPU.
# Output: outputs/step3/tau{τ}_beta{β}_seed{seed}.csv  (one file per run)
    ...
    for beta in "${BETAS[@]}"; do
        for seed in "${SEEDS[@]}"; do
            ...
            python scripts/train_delayed_sgd.py \
                ...
                --weight-decay 1e-4    \
                --data-root    data    \
                --out-dir      outputs/step3
        done
    done
done

echo ""
echo "All ${total} runs complete. Results in outputs/step3/"
echo "Run:  python scripts/analyze_step3.py"
